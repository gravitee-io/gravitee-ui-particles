...
import {
  fakeAllPolicies,
  fakeLlmFlow,
  fakeLlmProxyEndpoint,
  fakeLlmProxyEntrypoint,
  fakePlan,
  fakeTestPolicy,
  fakeTestPolicyStep,
...
describe('GioPolicyStudioComponent - LLM Proxy', () => {
  ...
  beforeEach(async () => {
    ...
    component.apiType = 'LLM_PROXY';
    fixture.detectChanges();
  });

  describe('with entrypointsInfo & endpointsInfo', () => {
    beforeEach(() => {
      component.entrypointsInfo = [fakeLlmProxyEntrypoint()];
      component.endpointsInfo = [fakeLlmProxyEndpoint()];
      component.ngOnChanges({
    ...
    it('should display top bar', async () => {
      ...
      await tooltip.show();
      expect(await tooltip.getTooltipText()).toEqual(`Entrypoints: LLM Proxy\nEndpoints: LLM Proxy`);
    ...
    it('should display flow detail info bar', async () => {
      ...
      expect(await detailsHarness.getFlowInfos()).toEqual({ 'LLM Methods': ['All Methods'], Name: ['Flow 1'] });
    });
    ...
    it('should add flow into common flows', async () => {
      ...
      await policyStudioHarness.addFlow('All plans', fakeLlmFlow({ name: '' }));
    ...
    it('should edit flow into plan', async () => {
      const planFooFlows = [fakeLlmFlow({ name: 'Foo flow 1' })];
