...
import {
  fakeAllPolicies,
  fakeA2aFlow,
  fakeA2aProxyEndpoint,
  fakeA2aProxyEntrypoint,
  fakePlan,
  fakeTestPolicy,
  fakeTestPolicyStep,
...
describe('GioPolicyStudioComponent - A2A Proxy', () => {
  ...
  beforeEach(async () => {
    ...
    component.apiType = 'A2A_PROXY';
    fixture.detectChanges();
  });

  describe('with entrypointsInfo & endpointsInfo', () => {
    beforeEach(() => {
      component.entrypointsInfo = [fakeA2aProxyEntrypoint()];
      component.endpointsInfo = [fakeA2aProxyEndpoint()];
      component.ngOnChanges({
    ...
    it('should display top bar', async () => {
      ...
      await tooltip.show();
      expect(await tooltip.getTooltipText()).toEqual(`Entrypoints: A2A Proxy\nEndpoints: A2A Proxy`);
    ...
    it('should add flow into common flows', async () => {
      ...
      await policyStudioHarness.addFlow('All plans', fakeA2aFlow({ name: 'All plans flow' }));
    ...
    it('should edit flow into plan', async () => {
      const planFooFlows = [fakeA2aFlow({ name: 'Foo flow 1' })];
